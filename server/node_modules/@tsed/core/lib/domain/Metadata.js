...
class Metadata {
    ...
    static get(key, target, propertyKey) {
        return Reflect.getMetadata(key, utils_1.classOf(target), propertyKey);
    }
    /**
     * Gets the metadata value for the provided metadata key on the target object or its prototype chain.
     * @param key A key used to store and retrieve metadata.
    ...
    static getOwn(key, target, propertyKey) {
        return Reflect.getOwnMetadata(key, utils_1.classOf(target), propertyKey);
    }
    /**
    ...
    static has(key, target, propertyKey) {
        try {
            return Reflect.hasMetadata(key, utils_1.classOf(target), propertyKey);
        }
        catch (er) { }
        return false;
    }
    ...
    static hasOwn(key, target, propertyKey) {
        return Reflect.hasOwnMetadata(key, utils_1.classOf(target), propertyKey);
    }
    /**
    ...
    static delete(key, target, propertyKey) {
        return Reflect.deleteMetadata(key, utils_1.classOf(target), propertyKey);
    }
    /**
    ...
    static set(key, value, target, propertyKey) {
        ...
        const classConstructor = utils_1.classOf(target);
        if (targets.indexOf(classConstructor) === -1) {
            targets.push(classConstructor);
            PROPERTIES.set(key, targets);
        }
        Reflect.defineMetadata(key, value, utils_1.classOf(target), propertyKey);
    }
    /**
